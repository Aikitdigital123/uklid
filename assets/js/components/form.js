...
function scrollStatusIntoView(statusNode) {
  if (!statusNode || typeof statusNode.scrollIntoView !== 'function') return;
...
export function initForms() {
  ...
  if (contactForm && formStatusContact) {
    contactForm.addEventListener('submit', async (event) => {
      ...
      if (submitButton) {
        ...
        submitButton.dataset.originalText = submitButton.textContent || 'Odeslat poptávku';
  ...
  if (calcForm && formStatusCalc) {
    calcForm.addEventListener('submit', async (event) => {
      ...
      calcForm.setAttribute('aria-busy', 'true');
      if (submitButton) {
        submitButton.disabled = true;
        submitButton.classList.add('is-loading');
        submitButton.dataset.originalText = submitButton.textContent || 'Odeslat nezávaznou poptávku';
        submitButton.textContent = '';
        const spinner = document.createElement('span');
        spinner.className = 'btn-spinner';
        spinner.setAttribute('aria-hidden', 'true');
      ...
      try {
        ...
        if (result.success) {
          ...
          calcForm.reset();
          const cleaningTypeSelect = calcForm.querySelector('#cleaningType');
          if (cleaningTypeSelect) cleaningTypeSelect.dispatchEvent(new Event('change', { bubbles: true }));
          markFormStart(calcForm);
        ...
        if (submitButton) {
          submitButton.disabled = false;
          submitButton.classList.remove('is-loading');
          submitButton.textContent = submitButton.dataset.originalText || 'Odeslat nezávaznou poptávku';
          delete submitButton.dataset.originalText;
        }
